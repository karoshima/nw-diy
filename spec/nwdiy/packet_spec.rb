...
# end
#
# 上記のように 「"def_field" 型, フィールド名, オプション」あるいは
# 「"def_array" 型, 個数, フィールド名, オプション」と定義します。
...
#
#   compile: lambda { xxx }
#     フィールドを参照したとき、自動計算フラグ compile_flag が
#     偽であれば設定値をそのまま返しますが、
#     真であれば設定値ではなく lambda 式の結果を返します。
#
#   length: 配列サイズ
...
#    テンプレート文字列を返します。
#    省略時値は def_field, def_array などから自動生成します。
#    変更したくなると思えないので、隠しメソッドでいいかな・・・
#
# calc_cksum(*args) -> Integer
...
#
# フィールド名 -> 型に対応した値の配列
#    def_array で定義したフィールド名に設定された値の配列を返します。
#    自動コンパイルはありません。使う場面ないんじゃないかなあ。
#    使う場面でてきたら、また考えます。
#
# フィールド名[n] -> 型に対応した値
#    def_array で定義したフィールド名の n 番目に設定された値を返します。
#    自動コンパイルは、ありません (同上)。
#
...
#
# フィールド名=([value, value, ...]) -> [値, 値, ...]
#    value が Array であれば
#    def_array で定義したフィールド配列に value を設定して返します。
#
# フィールド名[n]=value
#    def_array で定義した配列の n 番目に value を設定して返します。
#    value が String であれば、その余った部分も返します。
#
# compile_flag=flag
#    自動計算のするかどうかを設定します。
#    未設定のときは真となっています。
